...
document.querySelectorAll('.btn-login').forEach(button => {
  button.addEventListener('click', function () {
    showNotification(
      'Login functionality coming soon! Please contact us for account access.',
      'info'
    );
  });
});
...
document.addEventListener('DOMContentLoaded', function () {
  const animatedElements = document.querySelectorAll(
    '.feature-card, .service-card, .team-member, .value-card, .achievement-item, .faq-item'
  );
...
// Form loading function (can be used for future enhancements)
...
function showFieldError(field, message) {
  ...
  errorDiv.style.cssText =
    'color: #f56565; font-size: 0.875rem; margin-top: 0.25rem;';
